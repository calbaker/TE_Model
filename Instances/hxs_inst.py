...
hxs.x_dim = np.linspace(0., hxs.length, hxs.nodes)

plt.figure()
plt.plot(hxs.x_dim * 100., hxs.exh.T_nodes, '-r', label='Exhaust')
plt.plot(hxs.x_dim * 100., hxs.te_pair.T_h_nodes, '-.g', label='TE_PAIR Hot Side')
plt.plot(hxs.x_dim * 100., hxs.te_pair.T_c_nodes, '-.k', label='TE_PAIR Cold Side')
plt.plot(hxs.x_dim * 100., hxs.cool.T_nodes, '-b', label='Coolant')
...
plt.figure()
plt.plot(hxs.x_dim * 100., hxs.te_pair.power_nodes, 's', label='Exhaust')
plt.xlabel('Distance Along HX (cm)')
...
plt.figure()
plt.plot(hxs.x_dim * 100., hxs.exh.availability_flow_nodes, label='exhaust')
plt.plot(hxs.x_dim * 100., hxs.cool.availability_flow_nodes, label='coolant')
plt.xlabel('Distance Along HX (cm)')
