        ...
        while sp.absolute(self.T[-1] - self.Th) > self.err: # condition such
            ...
            for i in sp.arange(1,self.segments):
                ...
                self.k[i-1] = 100. * 3.194 / self.T[i-1] # p-type conducitivity
                                        # UNITS ???
                self.T[i] = ( self.T[i-1] + (self.dx / self.k[i-1]) *
                (self.J * self.T[i-1] * self.S[i-1] - self.q[i-1]) )
                # finite difference for temperature eq.
                
                self.q[i] = ( self.q[i-1] + (self.rho * self.J**2 * (1
                + self.S[i-1]**2 * self.T[i-1] / (self.rho *
                self.k[i-1])) - self.J * self.S[i-1] * self.q[i-1] /
                self.k[i-1]) * self.dx ) #finite 
                                        #difference  
                                        #for heat flux eq.  Chad
                                        #changed J * J to J**2
                self.dT = self.T[i] - self.T[i-1] # temperature (K)
                ...
                self.R_segment[i] = self.rho * self.dx  # resistance (ohms ???)
                                        # of segment
                self.q_c = self.q_c * (1 + (self.Th - self.T[-1]) /
                self.Th) # updates guess value of q_c
        # outside of loops
